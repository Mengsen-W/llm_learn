    ...
    def forward(self, hidden_states, use_kv_cache=False):
        ...
        if use_kv_cache and self.eval():
            if self.k_cache is None or self.k_cache.shape[1] != s - 1:
                ...
                q = torch.cat(
                    (torch.zeros_like(hidden_states[:, :-1, :]), self.q_proj(token)),
                    dim=1,
                )
        ...
        if self.flash_attn:
            ...
            mask = torch.full(
                (1, 1, self.config.max_seq_len, self.config.max_seq_len), float("-inf")
            )  # 初始化掩码
...
def load_balancing_loss_func(gate_logits, num_experts, top_k):
    ...
    )  # 各个层的gate_logit进行合并[layers X batch_size X sequence_length, num_experts]
...
class Gating(nn.Module):
    ...
    def forward(self, x):
        ...
        logits_topk, indices = logits.topk(
            self.topk, dim=-1
        )  # 选择概率最大的两个专家，返回两个专家对每个token的概率
        zeros = torch.full_like(
            logits, float("-inf")
        )  # 创建一个全为负无穷的矩阵，用于屏蔽其他专家的概率并重新归一化概率最大的两个专家
        sparse_logits = zeros.scatter(
            dim=-1, index=indices, src=logits_topk
        )  # 将选择的两个专家的概率按指定索引填充
        sparse_logits = F.softmax(
            sparse_logits, dim=-1
        )  # 得到一个稀疏矩阵，选择的两个专家对每个token的概率和为1
...
class MoE(nn.Module):
    ...
    def forward(self, x):
        ...
        sparse_logits_flat = sparse_logits.view(
            -1, sparse_logits.shape[-1]
        )  # (batch_size * seq_len, export_num))
        ...
        for i, expert in enumerate(self.experts):
            ...
            if expert_mask_flat.any():
                ...
                gate_scores = sparse_logits_flat[expert_mask_flat, i].unsqueeze(
                    1
                )  # (seq_true) --> (seq_true, 1)
...
class DecoderLayer(nn.Module):
    ...
    def forward(self, hidden_states, use_kv_cache):
        ...
        hidden_states = self.self_attn(
            hidden_states=hidden_states, use_kv_cache=use_kv_cache
        )
...
class LLM(PreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.tokon_embeddings = nn.Embedding(
    ...
    def forward(self, input_ids, labels, use_kv_cache=False):
        ...
        if self.config.output_router_logits:
            self.aux_loss = load_balancing_loss_func(
                all_router_logits, self.expert_num, self.topk
            )

            if labels is not None:
                self.loss += self.config.aux_loss_coef * self.aux_loss.to(
                    self.loss.device
                )
...
if __name__ == "__main__":
    ...
    model = LLM(config)
    print(
        f"模型参数量为：{sum(p.numel() for p in model.parameters() if p.requires_grad)}"
    )

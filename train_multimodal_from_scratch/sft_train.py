        ...
        try:
            ...
            for conversation in conversations:
                if conversation["from"] == "human":
                    ...
                    messages.append(
                        {"role": "assistant", "content": conversation["value"]}
                    )
            ...
            image = Image.open(os.path.join(self.images_path, image_name)).convert(
                "RGB"
            )
            ...
            pixel_values = self.processor(text=None, images=default_image)[
                "pixel_values"
            ]
...
class MyDataCollator:
    ...
    def __call__(self, features: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        for feature in features:
            input_ids.append(
                feature["input_ids"]
                + [self.tokenizer.pad_token_id] * (max_len - len(feature["input_ids"]))
            )
            labels.append(
                feature["labels"]
                + [self.tokenizer.pad_token_id] * (max_len - len(feature["labels"]))
            )
...
if __name__ == "__main__":
    ...
    model = AutoModelForCausalLM.from_pretrained(
        "train_multimodal_from_scratch/save/pretrain"
    )
